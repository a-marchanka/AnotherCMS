...
{if $galery_list[item].filepath eq 'content/images/'}
	<div class="w3-left w3-margin-right w3-margin-bottom">
	<a href="javascript:selectFile('{$details.field_name}','../{$galery_list[item].filepath}{$galery_list[item].filename}')">
	<img src="../{$galery_list[item].filepath}thumbs/{$galery_list[item].filename}" alt="{$galery_list[item].filename}" title="{$galery_list[item].filename}" width="95" height="95" class="w3-card-2"></a>
	</div>
{/if}
{/section}
<!-- list -->
...
<div class="w3-row w3-margin-top w3-margin-bottom">
	{if $pager_list.page ne 1}
		<a href="javascript:pagerGo('form_pager_header','first','browser');" class="w3-border w3-light-grey w3-round w3-padding-small w3-margin-right-8" style="text-decoration:none">{#LB_first#}</a>
	{else}
		<span class="w3-border w3-text-grey w3-round w3-padding-small w3-margin-right-8">{#LB_first#}</span>
	{/if}
	{if $pager_list.page ne 1}
		<a href="javascript:pagerGo('form_pager_header','prev','browser');" class="w3-border w3-light-grey w3-round w3-padding-small w3-margin-right-8" style="text-decoration:none">{#LB_prev#}</a>
	{else}
	...
	<select name="page" id="page" onchange="pagerGo('form_pager_header','search','browser');" class="w3-select w3-border w3-round w3-padding-small" style="width:auto;">
	...
	<input name="subaction" type="hidden" value="browser">
	...
	{if $pager_list.page ne $pager_list.pages}
		<a href="javascript:pagerGo('form_pager_header','next','browser');" class="w3-border w3-light-grey w3-round w3-padding-small w3-margin-right-8" style="text-decoration:none">{#LB_next#}</a>
	{else}
		<span class="w3-border w3-text-grey w3-round w3-padding-small w3-margin-right-8">{#LB_next#}</span>
	{/if}
	{if $pager_list.page ne $pager_list.pages}
		<a href="javascript:pagerGo('form_pager_header','last','browser');" class="w3-border w3-light-grey w3-round w3-padding-small w3-margin-right-8" style="text-decoration:none">{#LB_last#}</a>
	{else}
