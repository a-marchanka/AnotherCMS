...
<script>
function oDel(uid) {
oCnfr('delete','guestbook',uid,'{/literal}{#LB_delete#}{literal} #'+uid+'?');
}
</script>
{/literal}
...
<div class="w3-row">
	...
	<input id="TnewObj1" class="w3-button w3-margin-right-8 w3-margin-bottom-8 w3-theme" type="button" onclick="rScr('details','guestbook')" value="{#LB_newMessage#}">
	{/if}
	</div>
</div>
...
<th>{#LB_id#}</th>
<th>{#LB_name#}</th>
<th class="w3-hide-small w3-hide-medium">{#LB_message#}</th>
<th class="w3-hide-small w3-hide-medium">{#LB_webPage#}</th>
<th class="w3-hide-small">{#LB_email#}</th>
<th class="w3-hide-small">{#LB_modified#}</th>
<th class="w3-center">{#LB_status#}</th>
<th class="w3-center">{#LB_delete#}</th>
</tr>
{section name=item loop=$guestbook_list}
	<tr>
	<td>{$guestbook_list[item].id}{if $guestbook_list[item].status == 3} *{/if}</td>
	<td>
	<a href="javascript:rScr('details','guestbook&amp;details_id={$guestbook_list[item].id}');" style="text-decoration:none"{if $guestbook_list[item].status < 2} class="w3-text-dark-grey"{/if}><span class="i20s_ed"></span>
	{$guestbook_list[item].name|truncate:20}</a>
	</td>
	<td class="w3-hide-small w3-hide-medium">{$guestbook_list[item].message|nl2br|truncate:70}</td>
	<td class="w3-hide-small w3-hide-medium">
	{if $guestbook_list[item].menu_id > 0}
		{section name=category loop=$search_list}
		{if $guestbook_list[item].menu_id eq $search_list[category].id}
		{$search_list[category].title}
		{/if}
		{/section}
	{else}--{/if}
	</td>
	<td class="w3-hide-small"><a href="mailto:{$guestbook_list[item].email}"><span class="i20s_em w3-hide-large" title="{#LB_email#}"></span>
	<span class="w3-hide-medium">{$guestbook_list[item].email|truncate:35}</span></a></td>
	<td class="w3-hide-small">{$guestbook_list[item].modifytime|date_format:"%d.%m.%Y %H:%M"}</td>
	<td style="text-align:center; text-align:-webkit-center; text-align:-moz-center;">
	{if $guestbook_list[item].status == 5}
	<span class="i20s_canc" title="{#LB_off#}"></span>
       	{elseif $guestbook_list[item].status == 4}
       	<span class="i20s_cont" title="{#LB_contact#}"></span>
	{elseif $guestbook_list[item].status == 3}
       	<span class="i20s_new" title="{#LB_new#}"></span>
	{elseif $guestbook_list[item].status == 2}
	<span class="i20s_act" title="{#LB_active#}"></span>
	{elseif $guestbook_list[item].status == 1}
	<span class="i20s_inact" title="{#LB_inactive#}"></span>
	{else}--{/if}
	</td>
	<td style="text-align:center; text-align:-webkit-center; text-align:-moz-center;">
	{if $entry_info.entry_attr > 2}
		<a href="javascript:oDel('{$guestbook_list[item].id}');" class="i20s_del" title="{#LB_delete#}"></a>
	{else}
...
<div class="w3-row">
{if $entry_info.entry_attr > 2} &nbsp;<br>
<input id="newObj1" class="w3-button w3-margin-right-8 w3-margin-bottom-8 w3-theme" type="button" onclick="rScr('details','guestbook')" value="{#LB_newMessage#}">
{/if}
</div>
