...
<div class="w3-row">
	...
	<input id="TnewObj1" class="w3-button w3-margin-right-8 w3-margin-bottom-8 w3-theme" type="button" onclick="rScr('details','delivery')" value="{#LB_newCode#}">
	{/if}
	</div>
</div>
...
<th>{#LB_country#}</th>
<th class="w3-hide-small">{#LB_price#}</th>
...
{section name=item loop=$items_list}
	...
	<td><a href="javascript:rScr('details','delivery&amp;details_id={$items_list[item].id}');" style="text-decoration:none"{if $items_list[item].enabled == 0} class="w3-text-dark-grey"{/if}><span class="i20s_ed"></span>{$items_list[item].code}</a></td>
	<td class="w3-hide-small w3-hide-medium">{$items_list[item].title}</td>
	<td title="{$items_list[item].country}">{$items_list[item].country_code}</td>
       	<td class="w3-hide-small">
       	{$items_list[item].weight_kg1}{#LB_kg#}:{$items_list[item].price_kg1_str}, {$items_list[item].weight_kg2}{#LB_kg#}:{$items_list[item].price_kg2_str}, {$items_list[item].weight_kg3}{#LB_kg#}:{$items_list[item].price_kg3_str}, {$items_list[item].weight_kg4}{#LB_kg#}:{$items_list[item].price_kg4_str}<br>
       	{$items_list[item].length_cm1}{#LB_cm#}:{$items_list[item].price_cm1_str}, {$items_list[item].length_cm2}{#LB_cm#}:{$items_list[item].price_cm2_str}, {$items_list[item].length_cm3}{#LB_cm#}:{$items_list[item].price_cm3_str}, {$items_list[item].length_cm4}{#LB_cm#}:{$items_list[item].price_cm4_str}
       	</td>
...
<div class="w3-row">
{if $entry_info.entry_attr > 2} &nbsp;<br>
<input id="newObj1" class="w3-button w3-margin-right-8 w3-margin-bottom-8 w3-theme" type="button" onclick="rScr('details','delivery')" value="{#LB_newCode#}">
{/if}
</div>
