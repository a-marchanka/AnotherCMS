...
<script>
function oDel(uid) {
oCnfr('delete','dsc',uid,'{/literal}{#LB_delete#}{literal} #'+uid+'?');
}
</script>
{/literal}
...
<div class="w3-row">
	...
	<input id="TnewObj1" class="w3-button w3-margin-right-8 w3-margin-bottom-8 w3-theme" type="button" onclick="rScr('details','discount')" value="{#LB_newCode#}">
	<input id="TnewObj2" class="w3-button w3-margin-right-8 w3-margin-bottom-8 w3-theme" type="button" onclick="oModul('125','list')" value="{#LB_orders#}">
	{/if}
	</div>
</div>
...
<th>{#LB_id#}</th>
<th>{#LB_code#}</th>
<th class="w3-hide-small w3-hide-medium">{#LB_title#}</th>
<th>{#LB_discount#}</th>
<th class="w3-hide-small">{#LB_type#}</th>
<th class="w3-hide-small w3-hide-medium">{#LB_validFrom#}</th>
<th class="w3-hide-small">{#LB_validTo#}</th>
...
{section name=item loop=$items_list}
	...
	<td title="{$items_list[item].title}"><a href="javascript:rScr('details','discount&amp;details_id={$items_list[item].id}');" style="text-decoration:none"{if $items_list[item].enabled == 0} class="w3-text-dark-grey"{/if}><span class="i20s_ed"></span>{$items_list[item].code}</a></td>
	<td class="w3-hide-small w3-hide-medium">{$items_list[item].title}</td>
	<td title="{$items_list[item].price_type_str}">{$items_list[item].price_str}
	{if $items_list[item].pattern}
		<a href="javascript:oDtl('preview','{$items_list[item].id}');" title="{#LB_preview#}" class="i20s_pre"></a>
	{else}
		<span class="i20s_pre0" title="{#LB_preview#}"></span>
	{/if}
	</td>
       	<td class="w3-hide-small">{$items_list[item].price_type_str} {if $items_list[item].reusable > 0} &nbsp;{#LB_reusable#}{/if}</td>
	<td class="w3-hide-small w3-hide-medium">{if $items_list[item].active_from}{$items_list[item].active_from|date_format:"%d.%m.%Y"}{else}--{/if}</td>
	<td class="w3-hide-small">{if $items_list[item].active_to}{$items_list[item].active_to|date_format:"%d.%m.%Y"}{else}--{/if}</td>
...
<div class="w3-row">
{if $entry_info.entry_attr > 2} &nbsp;<br>
<input id="newObj1" class="w3-button w3-margin-right-8 w3-margin-bottom-8 w3-theme" type="button" onclick="rScr('details','discount')" value="{#LB_newCode#}">
<input id="newObj2" class="w3-button w3-margin-right-8 w3-margin-bottom-8 w3-theme" type="button" onclick="oModul('125','list')" value="{#LB_orders#}">
{/if}
</div>
