...
if (!$www_dir || !$site_usr || !$site_pwd) {
	$error_msg = 'Not all mandatory fields from step 1 have been completed.';
} else {
	...
	$warning_msg .= 'Database configuration was saved successfully.';
	$step = 2;
}
...
if (!$site_url||!$site_alias||!$site_title||!$site_description||!$site_keywords||!$site_dir||!$email) {
	$error_msg .= '<br />Not all mandatory fields from step 2 have been completed.';
} else {
	// get template and replace variables
	$sql_tpl = replaceContentWithValues(getContentFromPath($www_dir.'/setup/setup-db-en.tpl'), $_POST);
	...
	for ($i = 0; $i < sizeof($sql_array); $i ++) {
		...
		if (!$result)
			$error_msg .= '<br />Incorrect query: '.$i.' '.$db_link->lastErrorMsg().'<p><textarea cols="80" rows="5">'.$sql_array[$i].'</textarea></p>';
	}
	if (!$error_msg) $warning_msg .= '<br />Configuration of the website was saved successfully.';
	$step = 3;
}
}
...
if ($step == 3) {
if (!$web_server) {
	$error_msg .= '<br />Not all mandatory fields from step 3 have been completed.';
} else {
	...
	if ($result) {
		if ($fd2 = fopen($conf_2, 'w')) {
			...
			if (fwrite($fd2, $f_cms) === FALSE)
				$error_msg .= '<br />File '.$conf_2.' cannot be saved. ';
			else
				$warning_msg .= '<br />File '.$conf_2.' has been updated successfully. ';
			fclose($fd2);
		}
		if ($fd3 = fopen($conf_3, 'w')) {
			//print $file_path;
			if (fwrite($fd3, $fa_site) === FALSE)
				$error_msg .= '<br />File '.$conf_3.' cannot be saved. ';
			else
				$warning_msg .= '<br />File '.$conf_3.' has been updated successfully. ';
			fclose($fd3);
		}
		if ($fd4 = fopen($conf_4, 'w')) {
			//print $file_path;
			if (fwrite($fd4, $fa_cms) === FALSE)
				$error_msg .= '<br />File '.$conf_4.' cannot be saved. ';
			else
				$warning_msg .= '<br />File '.$conf_4.' has been updated successfully. ';
			fclose($fd4);
		}
		if ($fd5 = fopen($conf_5, 'w')) {
			...
			if (fwrite($fd5, $fa_pwd) === FALSE)
				$error_msg .= '<br />File '.$conf_5.' cannot be saved. ';
			else 
				$warning_msg .= '<br />File '.$conf_5.' has been updated successfully. ';
			fclose($fd5);
		}
	}
}
...
?>
<form method="post" action="index-en.php" style="margin:0; padding:0">
<div id="searchPanel" class="content">
<h3>1. Configure the database</h3>
    <table border="0">
      <tr>
        <td width="180"> User name <span style="color:#FF0000">*</span>:</td>
        <td>
          <input name="site_usr" type="text" value="<?php echo $site_usr; ?>" size="40" maxlength="255" required />
          (Administrator) </td>
      </tr>
      <tr>
        <td width="180"> Password <span style="color:#FF0000">*</span>:</td>
        <td>
...
<h3>3. Webserver configure</h3>
	<div id="s_apache" style="display:none;">
	Setup will try to automatically update the files. If the error message appears, please replace the contents of the following files with newly generated data.
	<table border="0">
      <tr>
        <td>
	...
	Please replace the contents of the following file with newly generated data. Or copy the setup/lighttpd.conf.tpl file to /etc/lighttpd/lighttpd.conf of your web server.
	<table border="0">
      <tr>
        <td>
...
<div id="searchPanel" class="content">
<h3>5. Login and cleanup</h3>
	After logging in, you will find a link called Configuration, with which you can further customize the CMS.
	<br />
	<form action="../cms/index.php" method="post" id="Login" target="_blank">
        ...
        <td>&nbsp; &nbsp;</td>
        <td valign="top"><p><strong>Website:</strong></p><a href="<?php echo $site_url;?>" target="_blank"><?php echo $site_url;?></a></td>
        </tr></table>
	</form>
<h3 style="color:#dd0000">Important: In the root directory, please delete <strong>setup/</ strong> directory completely.</h3>
