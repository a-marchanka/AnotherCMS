<p>&nbsp;</p>
<h2 class="w3-center">Expressions</h2>
{section name=item loop=$galery_list}
  <img class="slideList w3-animate-opacity w3-card" style="margin-left:20%; width:60%; display:none;" src="{$galery_list[item].filepath}{$galery_list[item].filename}" alt="{$galery_list[item].alttext}">
{/section}
<p>&nbsp;</p>
...
<form name="newsl" method="post" action="#footerId" onsubmit="getSigEmail()">
  <div class="w3-section">
    <label style="color:#333; text-decoration:underline;">{#LB_email#}</label>
    <input class="w3-input" type="text" name="email" id="email" value="{$items.email|default:''}" style="max-width:430px;" required>
  </div>
  <div class="w3-section">
    <label class="w3-text-grey">{#LB_name#}</label>
    <input class="w3-input" type="text" name="name" value="{$items.name|default:''}" style="max-width:430px;">
  </div>
